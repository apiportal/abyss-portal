...
@Deprecated
public class SubjectPermissionServiceOld extends AbstractServiceOld<SubjectPermission> {

    private static Logger logger = LoggerFactory.getLogger(SubjectPermissionServiceOld.class);

    public SubjectPermissionServiceOld(Vertx vertx) throws Exception {
        super(vertx);
        logger.info("SubjectPermissionServiceOld() invoked " + vertx);
    }
    ...
    public Single<ResultSet> findAll() {
        logger.info("SubjectPermissionServiceOld findAll() invoked" + jdbcClient);
        return jdbcClient
                .rxGetConnection().flatMap(conn -> conn
                        ...
                        .flatMap(resultSet -> {
                            logger.trace("SubjectPermissionServiceOld findAll() # of records :[" + resultSet.getNumRows() + "]");
    ...
    public Single<ResultSet> filterByPermissionName(String permissionName) {
        logger.info("SubjectPermissionServiceOld filterByPermissionName() invoked" + jdbcClient);
        return jdbcClient
                .rxGetConnection().flatMap(conn -> conn
                        .setQueryTimeout(Config.getInstance().getConfigJsonObject().getInteger(Constants.PORTAL_DBQUERY_TIMEOUT))
                        // Disable auto commit to handle transaction manually
                        ...
                        .flatMap(resultSet -> {
                            logger.trace("SubjectPermissionServiceOld filterByPermissionName() # of records :[" + resultSet.getNumRows() + "]");
                            return Single.just(resultSet);
                        })
                        // close the connection regardless succeeded or failed
                        .doAfterTerminate(conn::close)
